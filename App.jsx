...
import { 
  ...
  Activity, LineChart as ChartIcon, Info, Download, Book, ExternalLink, Users2,
  PieChart as PieIcon, FileSearch
} from 'lucide-react';

// --- CONFIGURACIÓN DE IA ---
const apiKey = ""; 
const SYSTEM_PROMPT = `Eres el Consultor Senior de Bud Advisors. Tienes acceso a fichas de empresas (Clear Channel, Exterior Plus) y tesis de expertos. Responde con un tono ejecutivo.`;
...
// --- BASE DE DATOS: LICITACIONES (INCLUYENDO ZARAGOZA) ---
const tendersDatabase = [
  {
    ...
    expiry: '2037',
    duration: '15 años',
    status: 'Cerrado/Activo',
    risk: 'Bajo (Largo Plazo)',
    value: '59.397.989 €',
    analysis: `Contrato de concesión administrativa integral. El Ayuntamiento no financia instalación ni mantenimiento; el operador asume riesgo y ventura. Incluye marquesinas, MUPIs (mín. 80 digitales) y relojes. Una restricción clave es que el 20% del inventario se reserva para comunicación institucional sin contraprestación.`,
    extraContracts: [
      { name: 'Tranvía Línea 1', operator: 'Clear Channel', expiry: '2035', detail: 'Explotación publicitaria en convoyes y paradas.' }
    ]
  },
  { id: 'mad-mu', city: 'Madrid', name: 'Mobiliario Urbano', operator: 'Clear Channel / Atresmedia', expiry: '2028', risk: 'Alto' },
  { id: 'aena-aer', city: 'Red AENA', name: 'Aeropuertos', operator: 'Exterior Plus', expiry: '2027', risk: 'Muy Alto' }
];

// --- BASE DE DATOS: COMPAÑÍAS (FICHA TÉCNICA CLARAL CHANNEL) ---
const companyDatabase = [
  {
    ...
    description: "Operador y gestor de activos de Publicidad Exterior. Foco histórico en mobiliario urbano y centros comerciales con una estrategia de transformación digital progresiva.",
    headquarters: "C/ Arturo Soria 336, 1ª planta, Madrid",
    management: [
      { name: "Jordi Sáez", role: "Consejero Delegado" },
      { name: "Maite Rodríguez", role: "Directora General Comercial" },
      { name: "Jaime Linaza", role: "CFO y Dir. Transformación Digital" },
    ...
    inventory: {
      ...
      digitalWeight: "32% del ingreso total (FY24)"
    },
    financials: {
      capex: "9,9 M€ (FY24)",
      marketShare: "23% estimada",
      digitalGrowth: "+630 pantallas (FY22-24)"
    }
  },
  {
    id: 'ext-plus',
    ...
    status: "Target",
    valuation: "Premium",
    description: "Líder en transporte (AENA) y digitalización de gran formato. Target crítico para consolidación.",
    inventory: { total: "25.000+", digital: "900+" }
  }
];

// --- BASE DE DATOS: EXPERTOS (ENTREVISTAS COMPLETAS) ---
...
Pero en Global no nos conformamos con multiplicar nuestro portfolio. Apostamos por un DOOH basado en data, tecnología y medición, porque medir no es solo contar, es demostrar. Sabemos que uno de los grandes retos del medio Exterior es cuantificar y cualificar resultados, y por eso hemos desarrollado soluciones, junto a Cuende Infometrics, que permiten evaluar la eficacia real de las campañas. En esa línea, apostamos por la medición de soportes en movimiento con Geobus y desde hace pocas semanas, estamos evaluando datos de soportes Indoor en estaciones de tren con Geostation. 

Este compromiso con la innovación también se ve reflejado en iniciativas del sector, como el último estudio de IAB España, elaborado por Ebiquity y patrocinado entre otros por Global. En él, se llega a destacar que es “el DOOH es el segundo medio digital más eficiente en la generación de ventas en el corto como en el largo plazo”. Un dato de un significado impresionante para nosotros, porque nos indica que la venta programática y el uso inteligente del dato ya son una realidad en nuestra propuesta.`,
    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 text-slate-900">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 pb-8">
        <div className="flex-1">
          ...
          <p className="text-slate-500 text-lg font-bold italic">Mercado OOH: Análisis Técnico 2025.</p>
        </div>
        ...
        {showInsight && (
          <div className="absolute inset-0 bg-slate-900/95 p-12 flex flex-col justify-center animate-in zoom-in-95 duration-300 z-50">
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <p className="text-xl font-bold text-white italic tracking-tight leading-relaxed mb-8">
                "El medio Exterior mantiene un <span className="text-[#ffd900]">crecimiento sostenido</span>: pasa de 350 M€ (2022) a 460,5 M€ (2025), con variaciones interanuales de <span className="text-[#ffd900]">+16,29%, +6,14% y +6,6%</span> y una tasa de crecimiento compuesto del <span className="text-[#ffd900]">9,5%</span> batiendo el crecimiento negativo del mercado publicitario general (-2,6%)."
              </p>
            </div>
          </div>
        )}
      ...
      <div className="bg-white rounded-[50px] border border-slate-200 p-12 md:p-20 shadow-sm mt-20">
         <div className="max-w-4xl mx-auto">
            <h3 className="text-4xl font-black uppercase tracking-tighter italic text-slate-900 mb-12 flex items-center gap-4"><Book className="text-[#ffd900]" /> Índice <span className="text-slate-400">Estratégico</span></h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               {[
                 { id: 'soportes', label: 'Tipos de Soportes', mod: '02' },
                 { id: 'licitaciones', label: 'Licitaciones & Plazas', mod: '03' },
                 { id: 'companias', label: 'M&A Targets', mod: '04' },
                 { id: 'expertos', label: 'Visión de Expertos', mod: '05' }
               ].map((item) => (
                 <button key={item.id} onClick={() => onNavigate(item.id)} className="group flex flex-col p-8 bg-slate-50 rounded-[40px] border border-slate-100 hover:bg-black transition-all text-left">
                    <span className="text-[10px] font-black uppercase text-slate-400 group-hover:text-[#ffd900] mb-4">Módulo {item.mod}</span>
                    <span className="text-2xl font-black italic text-slate-900 group-hover:text-white uppercase leading-none">{item.label}</span>
...
// --- COMPONENTE DETALLE DE COMPAÑÍA ---
const CompanyDetailView = ({ company, onBack }) => (
  <div className="max-w-6xl mx-auto bg-white rounded-[60px] shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-10 pb-20">
    ...
    <div className="p-10 md:p-20 text-slate-900">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
        <div className="lg:col-span-2 space-y-12">
          <div>
            <h2 className="text-7xl font-black tracking-tighter italic uppercase leading-[0.8] mb-8">{company.name}</h2>
            <p className="text-2xl text-slate-500 font-medium italic leading-relaxed">{company.description}</p>
          </div>
          
          <div className="p-10 bg-slate-50 rounded-[40px] border border-slate-200">
             <h4 className="font-black uppercase text-xs tracking-[0.3em] mb-8 text-slate-400">Equipo Directivo España</h4>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {company.management?.map((mgr, i) => (
                 <div key={i} className="flex flex-col">
                   <span className="text-lg font-black italic text-slate-900">{mgr.name}</span>
                   <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{mgr.role}</span>
                 </div>
               ))}
             </div>
          </div>
        ...
        <div className="space-y-8">
           <div className="p-10 bg-slate-900 rounded-[50px] text-white shadow-xl">
              <h4 className="text-[#ffd900] font-black uppercase text-[10px] mb-8 italic tracking-[0.3em]">Snapshot de Inventario</h4>
              <div className="space-y-6">
                 <div><p className="text-slate-400 text-[9px] uppercase font-black mb-1 italic">Total Caras</p><p className="text-3xl font-black italic">{company.inventory?.total}</p></div>
                 <div><p className="text-slate-400 text-[9px] uppercase font-black mb-1 italic">Soportes Digitales</p><p className="text-3xl font-black italic text-[#ffd900]">{company.inventory?.digital}</p></div>
                 <div><p className="text-slate-400 text-[9px] uppercase font-black mb-1 italic">Peso Ingreso Digital</p><p className="text-xl font-black italic">{company.inventory?.digitalWeight}</p></div>
              </div>
           </div>
           
           <div className="p-10 bg-[#ffd900] rounded-[50px] text-black">
              <h4 className="font-black uppercase text-[10px] mb-8 italic tracking-[0.3em]">Métricas de Mercado</h4>
              <div className="space-y-4 font-bold text-sm italic">
                <p>Cuota Mercado: {company.financials?.marketShare}</p>
                <p>CAPEX: {company.financials?.capex}</p>
                <p>Sede: {company.headquarters}</p>
              </div>
           </div>
        </div>
      </div>
...
// --- COMPONENTE DETALLE DE LICITACIÓN (ZARAGOZA) ---
const TenderDetailView = ({ tender, onBack }) => (
  <div className="max-w-6xl mx-auto bg-white rounded-[60px] shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-10 pb-20">
    <div className="p-10 bg-slate-900 text-white flex justify-between items-center">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors font-black text-xs uppercase tracking-widest"><ChevronLeft size={20} /> Volver a Licitaciones</button>
      <div className="bg-[#ffd900] text-black px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest italic tracking-[0.2em]">Public Asset Analysis</div>
    </div>
    <div className="p-10 md:p-20 text-slate-900">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
        <div className="lg:col-span-2 space-y-12">
          <div>
            <p className="text-[#ffd900] font-black uppercase text-xs tracking-widest mb-2 italic">{tender.city}</p>
            <h2 className="text-7xl font-black tracking-tighter italic uppercase leading-[0.8] mb-8">{tender.name}</h2>
            <div className="flex items-center gap-4 p-6 bg-slate-50 rounded-3xl border">
               <div className="w-12 h-12 bg-black text-white rounded-xl flex items-center justify-center font-black">JD</div>
               <div><p className="text-[10px] font-black uppercase text-slate-400">Operador Adjudicatario</p><p className="font-black italic text-lg">{tender.operator}</p></div>
            </div>
          </div>
          
          <div className="prose prose-slate max-w-none">
             <h4 className="text-xl font-black italic uppercase mb-6">Diagnóstico del Mercado</h4>
             <p className="text-lg text-slate-600 leading-relaxed font-medium italic">{tender.analysis}</p>
          </div>
          ...
          <div className="p-10 bg-slate-900 rounded-[40px] text-white">
             ...
             <div className="space-y-6">
                {tender.extraContracts?.map((ext, idx) => (
                  <div key={idx} className="flex justify-between items-center border-b border-white/10 pb-4">
                     <div><p className="font-black italic text-xl">{ext.name}</p><p className="text-xs text-slate-400">{ext.detail}</p></div>
                     <div className="text-right"><p className="font-black text-[#ffd900] italic">{ext.operator}</p><p className="text-[10px] uppercase text-slate-400">Vence: {ext.expiry}</p></div>
                  </div>
                ))}
             </div>
          </div>
        ...
        <div className="space-y-8">
           <div className="p-10 bg-slate-50 rounded-[50px] border shadow-sm">
              <h4 className="text-slate-400 font-black uppercase text-[10px] mb-8 italic tracking-[0.3em]">Dato Esencial</h4>
              <div className="space-y-6">
                 ...
                 <div><p className="text-slate-400 text-[9px] uppercase font-black mb-1 italic">Riesgo</p><p className="text-xl font-black italic text-emerald-600">{tender.risk}</p></div>
...
const App = () => {
  ...
  const renderContent = () => {
    ...
    switch (activeArea) {
      ...
      case 'licitaciones': return <TendersListView onSelect={setSelectedItem} />;
        ...
        <nav className="p-8 space-y-4 flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="pt-8 mt-8 border-t border-black/10">
             ...
             <div className="mt-4 flex justify-center">
                <img src="https://static.wixstatic.com/media/88e5e3_c20937499861486895bbba1fd101b94e~mv2.png" alt="Bud Advisors Logo" className="w-1/3 h-auto object-contain mix-blend-multiply opacity-90 contrast-125" />
             </div>
          </div>
        </nav>
      </aside>
...
// --- LISTADOS ADAPTADOS PARA SELECCIÓN ---
const CompaniesView = ({ onSelect }) => (
  <div className="space-y-10 animate-in fade-in duration-700 pb-20">
    <h2 className="text-6xl font-black text-slate-900 tracking-tighter leading-none mb-4 uppercase italic">M&A <span className="text-slate-500 not-italic uppercase tracking-normal">Targets</span></h2>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {companyDatabase.map((c) => (
        <div key={c.id} onClick={() => onSelect(c)} className="bg-white border border-slate-200 rounded-[40px] p-10 hover:shadow-2xl transition-all cursor-pointer group">
          <div className="flex justify-between items-center mb-8"><span className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-100">{c.status}</span><p className="text-xs font-black italic">Val: {c.valuation.split(' ')[0]}</p></div>
          <h3 className="text-3xl font-black italic uppercase text-slate-900 mb-4">{c.name}</h3>
          <p className="text-slate-500 font-medium italic line-clamp-2">{c.description}</p>
        </div>
      ))}
    </div>
  </div>
...
const TendersListView = ({ onSelect }) => (
  <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-slate-900">
    <h2 className="text-6xl font-black tracking-tighter leading-none mb-4 uppercase italic">Licitaciones <span className="text-slate-500 not-italic uppercase tracking-normal">Control</span></h2>
    <div className="bg-white border border-slate-200 rounded-[40px] overflow-hidden shadow-sm">
       <table className="w-full text-left font-sans">
         <thead className="bg-slate-900 text-white font-black"><tr><th className="p-6 text-[10px] font-black uppercase tracking-widest italic">Plaza</th><th className="p-6 text-[10px] font-black uppercase tracking-widest italic">Activo</th><th className="p-6 text-[10px] font-black uppercase tracking-widest italic text-right">Riesgo</th><th className="p-6"></th></tr></thead>
         <tbody className="divide-y divide-slate-100 italic">
           {tendersDatabase.map((t) => (
             <tr key={t.id} className="hover:bg-yellow-50 transition-colors cursor-pointer" onClick={() => onSelect(t)}>
               <td className="p-6 font-black uppercase text-sm">{t.city}</td>
               <td className="p-6 text-xs font-black">{t.name}</td>
               <td className="p-6 text-right font-black text-rose-600">{t.risk.split(' ')[0]}</td>
               <td className="p-6 text-right"><ChevronRight size={16} /></td>
             </tr>
           ))}
         </tbody>
       </table>
...
const ExpertsListView = ({ onSelect }) => (
  <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-slate-900">
    <h2 className="text-6xl font-black tracking-tighter leading-none mb-4 uppercase italic">Expert <span className="text-slate-500 not-italic uppercase tracking-normal">Vision</span></h2>
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 font-sans">
      {expertInterviews.map((expert) => (
        <div key={expert.id} onClick={() => onSelect(expert)} className="bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm hover:shadow-xl hover:border-black transition-all cursor-pointer group relative overflow-hidden">
          ...
          <div className="mt-8 pt-6 border-t border-slate-50 flex justify-between items-center text-slate-900 font-black text-[10px] uppercase italic tracking-widest">Leer Análisis Completo <ArrowRight size={16} /></div>
        </div>
      ))}
    </div>
  </div>
...
// Componentes estáticos simplificados
const AssetsView = () => (<div className="p-20 text-center"><h2 className="text-4xl font-black italic text-slate-300 uppercase">Sección Soportes Activa</h2></div>);
